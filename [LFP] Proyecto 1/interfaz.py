...
class Pantalla_Principal():
    ...
    def abrir_manual_tecnico(self):
        try:  
            path = 'https://acrobat.adobe.com/link/review?uri=urn:aaid:scds:US:855b4300-ab2d-3ddc-866b-d3aa8992a46e'
            webbrowser.open_new(path)
        except:
            print("No se pudo abrir el archivo")
    ...
    def getErrores(self):
        ...
        while lista_errores:
            error = lista_errores.pop(0)
            errores[str(wendy)] = error.operar(wendy)
            wendy += 1

        with tempfile.TemporaryDirectory() as tmpdirname:
            file_path = os.path.join(tmpdirname, "ERRORES_202112092.json")
